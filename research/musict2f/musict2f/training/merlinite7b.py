...
from musict2f.evaluation import calculate_metrics
from musict2f import dataloading


def train(
    feature: str,
    models_save_dir: Path | str,
    ...
    experiment = (
        f"{feature}-{model_name}-_tag_texts-from_{stringifier_method}-by_{dimension}"
    )

    data = dataloading.read_text_sets(
        data_dir,
        dimension,
        feature,
        stringifier_method,
    ...
    model = IBMmerliniteRegressor(
        model_name,
        target_column_name=feature,
        output_dir=models_save_dir.joinpath(experiment),
        logger=logger,
        train_epochs=10,
        train_batch_size=10,
        eval_batch_size=10,
