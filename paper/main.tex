...
Our dataset, which we call \emph{Jimmydj2000 Single-user} dataset, presents mean ($\mu$) and standard deviation($\sigma$) values that are comparable to the same values of the Spotify Audio Features Kaggle dataset,
...
\begin{table}[!ht]
      ...
      \caption{Audio features description of the Jimmydj2000 Single-user dataset.}\label{table:audio-features-stats}
            \centering
                  \begin{tabular}{@{}lll@{}}
                  \toprule
                  Feature           & $\mu$ & $\sigma$ \\
...
\begin{figure}[h!]
      ...
      \caption{Distribution of audio features in the Jimmydj2000 Single-user dataset.}
...
We trained three commonly used machine learning models to predict Spotify audio features from Last.fm tags by using the
